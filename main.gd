...
''' failed hdr attempt
		if get_window().get_output_max_linear_value() > 1.1:
			var shader_material = load("res://Backgrounds/hdr.tres")
			back.material = shader_material
			back.material.set_shader_parameter(
				"mult_map",
				"res://Backgrounds/multiplicationmaps/" + new_texture_path.get_file()
			)
'''
